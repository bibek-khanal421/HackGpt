...
class ChatApp:
    ...
    def chat(self, input_text, chat_memory, model, temperature, hack_prompt):
        """
...
def main():
    ...
    app = ChatApp()
    memory = LangChainMemory(connection_string=DATABASE_URL, session_id=st.session_state.current_session_name)
    chat_memory = memory.get_memory()
    ...
    if st.session_state.current_session_name:
        ...
        history = chat_memory.load_memory_variables({}).get("history", [])
        ...
        if user_input:
            ...
            with st.spinner("processing..."):
                response = app.chat(user_input, chat_memory, model, temperature, hack_prompt)
                with st.chat_message("ai"):
                    st.write(response)
    else:
